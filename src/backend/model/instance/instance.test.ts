...
const templ = BEGINNING_TEMPLATES.EMPLOYEE_ONBOARDING;

describe('Beginning Tests', () => {
	it('Should be able to create a new Beginning Instance', () => {
		expect(new Instance(templ, "Brad Wisler")).toBeTruthy();
	});
	it('Should store the name of the template it came from', () => {
		expect(new Instance(templ, "Amazon LLC").parentName).toEqual(templ.name);
	});
	it('Name should include relevent information', () => {
		const begInst = new Instance(templ, "Brad Wisler");
	...
	it('Should have a parenTemplateId of the template it came from', () => {
		...
		const tmpTempl = new Workflow();
		const begInst2 = new Instance(tmpTempl);
