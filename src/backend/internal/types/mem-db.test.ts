...
import { Step, Workflow, Instance } from '../../model';
import { BEGINNING_TEMPLATES, FakeSeedData } from './fake-seed-data';
import { RandomData } from './random-data';

describe('MemDb Tests', () => {
	describe('MemDb Tests', () => {
		it('Should be able to be created', () => {
			const memDb = new MemDb<Step>();
		...
		it('Should retrieve all things in map', () => {
			const s = new Workflow();
			const memDb = new MemDb<Workflow>();
			for(let i = 0; i < 100; i++) {
				memDb.store(new Workflow());
			}
			memDb.store(s);
